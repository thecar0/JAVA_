...
public class StudentController implements StudentProgram{
	...
	public void addSubject() {
		subList.add(new Subject("123", "java", "1" , "김모씨", "A"));
		subList.add(new Subject("456", "DB", "2" , "이모씨", "B"));
		subList.add(new Subject("789", "C언어", "3" , "박모씨", "C"));
		subList.add(new Subject("135", "javaScript", "4" , "최모씨", "D"));
		subList.add(new Subject("246", "python", "5" , "배모씨", "E"));
	}
	...
	public void intsertStudent(Scanner scan) {
		...
		for(int i = 0; i < student.size(); i++) {
			if(student.get(i).getStudentno().equals(studentno)) {
	...
	public void printStudentList() {
		...
		for(int i = 0; i < student.size(); i++) { //i가 제한되어야 해서 향상된 for문 안씀 (배열이 다 차있지 않아서)
	...
	public void searchStudent(Scanner scan) {
		...
		//학생번지 확인
		for(int i = 0; i < student.size(); i++) {
			if(student.get(i).getStudentno().equals(num)) {
				student.get(i).printInfo();
				student.get(i).printSubjectList();
			}
	...
	public void insertSubject(Scanner scan) {
		...
		for(int i=0; i<subList.size(); i++) {
			if(student.get(i).getStudentno().equals(num)) {
				...
				student.get(i).insertSubject(subList.get(subIndex-1));
			}
		}
