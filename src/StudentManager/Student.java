...
public class Student {
	...
	private Subject[] subjectList = new Subject [5];  //학생 1명당 본인이 수강신청한 과목의 목록
	...
	//2. 배열이 가득찼을 경우 => 새배열을 만들어 배열복사 => 배열주소 변경
	public void insertSubject(Subject sub) {
/*		//배열에 아무것도 없을떄 (또 써야해서 그냥 메서드로 뺌)
		if(sub == null) {
			return;
		}
		
		if(sub.getSubjectcode().isBlank()) {  //기본키 (DB)에 나오는 개념 => 코드로 확인
			System.out.println("수강정보가 없습니다."); return;
		}
*/		
		//2번
		if(cnt == this.subjectList.length) {
			Subject[] subTemp = new Subject[this.subjectList.length+3];
			System.arraycopy(this.subjectList, 0, subTemp, 0, this.subjectList.length);
			this.subjectList = subTemp;
		}
		
		//1번
		//cnt까지 배열을 순회하여 subjectcode가 일치하는 값이 있다면
		for(int i = 0; i < cnt; i++) {
	...
	public void removeSubject(Subject sub) {
		...
		//찾는값이 있을때
		//뒷번지 앞으로 옮기기 (index번지부터 cnt-1번지 까지)
		...
		cnt--;
		System.out.println("수강철회를 완료했습니다.");
	}

	
/*	//과목배열이 가득 차면 늘려주는 메서드
...
// 출력 메서드
	...
	public String toString() {
		return "Student [studentno=" + studentno + ", studentname=" + studentname + ", age=" + age + ", phone=" + phone
				+ ", address=" + address + ", subjectList=" + Arrays.toString(subjectList) + "]";
	}
